...
public class AuthService {
    ...
    public UserDetails loadUserByUsername(String login) throws UsernameNotFoundException {
        ...
        org.springframework.security.core.userdetails.User user = new org.springframework.security.core.userdetails.User( authResponse.getMessage()[0],
                authResponse.getMessage()[1],
                Collections.singleton(new SimpleGrantedAuthority("ROLE_" + authResponse.getMessage()[2])));
                logger.info("Loaded user: %s".formatted(user.toString()));
                return user;
    }
    ...
    public String registerUser(User user) throws JsonProcessingException, InterruptedException {
        ...
        logger.info("Sent user for registration with login and id: %s".formatted(user.toString()));
    ...
    private void sendUserCheckRequest(String login) {
        ...
        logger.info("Sent login(%s) to user-service".formatted(login));
    }
    ...
    public void userExistsByLogin(String jsonResponse) throws JsonProcessingException {
        ...
        logger.info("Received user, login: %s".formatted(login));
